...
macro_rules! id_type {
    ($name:ident, $doc:literal) => {
        #[doc = $doc]
        #[derive(Clone, Copy, Debug, Facet, PartialEq, Eq, PartialOrd, Ord, Hash)]
        pub struct $name(u32);
...
pub struct WeavySnarkProfileBackendExecutionSummary {
    ...
    pub execution: SnarkStencilExecution,
    /// Conservative union of parser and lexer session state in this execution lane.
    pub state: Vec<SnarkStencilState>,
...
impl WeavySnarkProfileStencilReadiness {
    ...
    pub fn backend_execution_summaries(&self) -> Vec<WeavySnarkProfileBackendExecutionSummary> {
        let mut counts = BTreeMap::<
            ...
            (usize, usize, BTreeSet<SnarkStencilState>, EffectContract),
        >::new();
        for summary in &self.execution_summaries {
            let (parser_count, _, state, effect) = counts.entry(summary.execution).or_default();
            *parser_count += summary.count;
            state.extend(summary.state.iter().copied());
            accumulate_effect_union(effect, summary.effect.clone());
        }
        for summary in &self.lexer_summaries {
            let (_, lexer_count, state, effect) = counts.entry(summary.execution).or_default();
        ...
        let mut summaries = counts
            .into_iter()
            .map(|(execution, (parser_count, lexer_count, state, effect))| {
...
impl ResolvedCstNamesData {
    fn from_runtime(names: &parser_ir::ResolvedCstNames) -> Self {
        let (eof, error, missing, recovery) = names.special_names();
        Self {
            fields: names.fields().map(str::to_owned).collect(),
            public_nodes: names.public_nodes().map(str::to_owned).collect(),
            aliases: names.aliases().map(str::to_owned).collect(),
            terminals: names.terminals().map(str::to_owned).collect(),
            nonterminals: names.nonterminals().map(str::to_owned).collect(),
            externals: names
                .externals()
    ...
    fn into_runtime(self) -> parser_ir::ResolvedCstNames {
        parser_ir::ResolvedCstNames::from_owned_parts(parser_ir::ResolvedCstNameParts {
            fields: self.fields,
...
fn loaded_runtime_weavy_parser_program(
    public_node_kind_names: Vec<String>,
    alias_names: Vec<String>,
    extra_nodes: Vec<RuntimeWeavyExtraNodeData>,
) -> WeavyParserProgram {
    WeavyParserProgram {
        lowered: empty_lowered(),
        dense: DenseSnarkWeavyLowered::new(Vec::new(), Vec::new()),
        ...
        extra_node_index: RuntimeWeavyExtraNodeIndex {
            nodes: extra_nodes
                ...
                .map(|node| {
                    (
                        node.lookahead,
                        RuntimeWeavyExtraNode {
                            kind: Arc::<str>::from(node.kind),
        ...
        public_node_kind_names: public_node_kind_names
            .into_iter()
            .map(Arc::<str>::from)
            .collect(),
...
impl WeavyParsePlan {
    ...
    pub fn new(
        ...
        let production_dynamic_precedence = parser
            .productions()
            .iter()
            .map(parser_ir::Production::dynamic_precedence)
            .collect();
    ...
    pub fn from_artifact_data(data: WeavyParsePlanData) -> Result<Self, WeavyParseError> {
        ...
        let alias_names = data.resolved_cst_names.aliases.clone();
        let program = loaded_runtime_weavy_parser_program(
...
pub struct WeavyLexToken {
    ...
    /// Keyword/reserved-word rewrite status for the selected token.
    pub keyword: parser_ir::KeywordStatus,
...
#[derive(Clone, Debug, Facet, PartialEq, Eq)]
struct WeavyLexerProgramData {
    modes: Vec<WeavyLexModeData>,
    mode_ids: Vec<u32>,
    state_mode_ids: Vec<u32>,
}

#[derive(Clone, Debug, Facet, PartialEq, Eq)]
struct WeavyLexModeData {
    terminals: Vec<WeavyLexTerminalData>,
    external_lookaheads: Vec<WeavyExternalLookaheadData>,
    external_count: usize,
}

#[derive(Clone, Debug, Facet, PartialEq, Eq)]
struct WeavyLexTerminalData {
    terminal: parser_ir::TerminalId,
    matcher: WeavyTerminalMatcherData,
    lookahead: Option<WeavyTerminalLookaheadData>,
    immediate: bool,
    literal: bool,
...
#[derive(Clone, Copy, Debug, Facet, PartialEq, Eq)]
struct WeavyTerminalLookaheadData {
    terminal: parser_ir::TerminalId,
    lookahead: parser_ir::LookaheadSymbol,
...
#[derive(Clone, Copy, Debug, Facet, PartialEq, Eq)]
struct WeavyExternalLookaheadData {
...
#[derive(Clone, Debug, Facet, PartialEq, Eq)]
#[repr(u8)]
enum WeavyTerminalMatcherData {
    Expr(WeavyLexExprData),
...
enum WeavyLexExprData {
    ...
    Repeat(Box<WeavyLexExprData>),
    Repeat1(Box<WeavyLexExprData>),
...
impl WeavyLexerProgram {
    ...
    fn artifact_data(&self) -> WeavyLexerProgramData {
        let mut modes = Vec::<WeavyLexModeData>::new();
        let mut intern = |mode: &WeavyLexModeProgram| {
            let data = mode.artifact_data();
            if let Some(index) = modes.iter().position(|candidate| candidate == &data) {
                u32::try_from(index).expect("lexer mode count exceeds u32")
            } else {
                let index = u32::try_from(modes.len()).expect("lexer mode count exceeds u32");
                modes.push(data);
                index
            }
        };
        ...
        let resolve = |mode: u32| {
            interned
                ...
                .ok_or(WeavyParseError::MissingLexMode {
                    mode: parser_ir::LexModeId::from_index(mode as usize),
                })
        };
        let modes = data
            .mode_ids
...
struct WeavyLexerCompiler {
    ...
    /// deduplicated compiled regex leaves by (source, flags). `compile_composite_regex`
    /// and pattern leaves used to build a fresh `AutomataRegex` + `HybridDfa` per lex
    /// mode even for identical sources; caching makes repeated leaves share one engine
    /// (and one runtime DFA cache slot, which is safe — the cache is a pure function of
    /// the source).
    regex_leaves: HashMap<(String, Option<String>), WeavyRegexLeaf>,
    /// memoized ε-membership per regex source. `regex_source_matches_empty` used to
    /// compile a full regex_automata engine per call just to test whether a pattern
    /// matches the empty string; on real grammars the same handful of sources recur
    /// across every lex mode, so a per-compiler cache removes essentially all of that
    /// compilation (tens of thousands of builds down to a handful).
    regex_emptiness: std::collections::HashMap<String, bool>,
}

impl WeavyLexerCompiler {
    /// Whether `source` matches the empty string, memoized per source so repeated
    /// ε-membership tests across lex modes never recompile the regex engine.
    fn regex_source_matches_empty(&mut self, source: &str) -> bool {
        if let Some(&result) = self.regex_emptiness.get(source) {
            return result;
        }
        let result = regex_source_matches_empty_compute(source);
        self.regex_emptiness.insert(source.to_string(), result);
        result
    }
}
...
impl WeavyLexerCompiler {
    ...
    fn direct_pattern_set(
        ...
        let entries = WeavyDirectPatternSet::entries_from_terminals(terminals, self);
        if entries.is_empty() {
            return None;
        }
    ...
    fn regex_leaf(&mut self, source: String, flags: Option<String>) -> Option<WeavyRegexLeaf> {
        if let Some(leaf) = self.regex_leaves.get(&(source.clone(), flags.clone())) {
            return Some(leaf.clone());
        }
        let id = self.next_regex_leaf_id;
        let leaf = WeavyRegexLeaf::new(Some(id), source.clone(), flags.clone())?;
        self.next_regex_leaf_id += 1;
        self.regex_leaves.insert((source, flags), leaf.clone());
        Some(leaf)
    }
...
impl WeavyLexModeProgram {
    ...
    fn artifact_data(&self) -> WeavyLexModeData {
        WeavyLexModeData {
            terminals: self
                .terminals
                .iter()
                .map(WeavyLexTerminal::artifact_data)
                .collect(),
            external_lookaheads: self
                .external_lookaheads
                .iter()
                .map(|lookahead| WeavyExternalLookaheadData {
                    external: lookahead.external,
    ...
    fn from_artifact_data(
        data: WeavyLexModeData,
        compiler: &mut WeavyLexerCompiler,
    ) -> Result<Self, WeavyParseError> {
        let terminals = data
            .terminals
            .into_iter()
            .map(|terminal| WeavyLexTerminal::from_artifact_data(terminal, compiler))
            .collect::<Result<Vec<_>, _>>()?;
        let external_lookaheads = data
            .external_lookaheads
            .into_iter()
            .map(|lookahead| RuntimeWeavyExternalLookahead {
                external: lookahead.external,
                lookahead: lookahead.lookahead,
            })
            .collect();
        Ok(Self::from_terminals(
            terminals,
            external_lookaheads,
            data.external_count,
            compiler,
...
impl WeavyLexTerminal {
    ...
    fn artifact_data(&self) -> WeavyLexTerminalData {
        WeavyLexTerminalData {
            ...
            matcher: self.matcher.artifact_data(),
            lookahead: self.lookahead.map(|lookahead| WeavyTerminalLookaheadData {
            ...
            lexical_precedence: self.lexical_precedence,
            implicit_precedence: self.implicit_precedence,
        }
    }

    fn from_artifact_data(
        data: WeavyLexTerminalData,
        compiler: &mut WeavyLexerCompiler,
    ) -> Result<Self, WeavyParseError> {
        Ok(Self {
            ...
            implicit_precedence: data.implicit_precedence,
            direct_literal_index: None,
            direct_pattern_index: None,
        })
    }
...
impl WeavyTerminalMatcher {
    ...
    fn from_artifact_data(
        data: WeavyTerminalMatcherData,
        compiler: &mut WeavyLexerCompiler,
    ) -> Result<Self, WeavyParseError> {
        Ok(match data {
            WeavyTerminalMatcherData::Expr(expr) => {
                Self::Expr(WeavyLexExpr::from_artifact_data(expr, compiler)?)
            }
            WeavyTerminalMatcherData::UnsupportedTerminal { terminal } => {
                Self::UnsupportedTerminal { terminal }
            }
        })
    }
...
impl WeavyLexExpr {
    fn from_compiled(value: parser_ir::CompiledLexExpr, compiler: &mut WeavyLexerCompiler) -> Self {
        let expr = match value {
            ...
            parser_ir::CompiledLexExpr::Pattern(pattern) => Self::Pattern {
                source: pattern.source.clone(),
                flags: pattern.flags.clone(),
                matcher: compiler.pattern_matcher(pattern),
            },
    ...
    fn compile_composite_regex(expr: Self, compiler: &mut WeavyLexerCompiler) -> Self {
        let Some(source) = regex_source_for_weavy_lex_expr(&expr, compiler) else {
            return expr;
        };
    ...
    fn compile_composite_choice(expr: Self, compiler: &mut WeavyLexerCompiler) -> Self {
        ...
        for member in members {
            let Some(source) = regex_source_for_weavy_lex_expr(member, compiler) else {
                return expr;
            };
            if source.is_empty() {
                return expr;
    ...
    fn artifact_data(&self) -> WeavyLexExprData {
        match self {
            ...
            Self::String(value) => WeavyLexExprData::String(value.clone()),
            Self::Pattern { source, flags, .. } => WeavyLexExprData::Pattern {
                source: source.clone(),
                flags: flags.clone(),
            },
            Self::Until(matcher) => WeavyLexExprData::Until {
                markers: matcher.markers.clone(),
            },
            Self::Nested { open, close } => WeavyLexExprData::Nested {
                open: open.clone(),
                close: close.clone(),
            },
    ...
    fn from_artifact_data(
        data: WeavyLexExprData,
        compiler: &mut WeavyLexerCompiler,
    ) -> Result<Self, WeavyParseError> {
        let expr = match data {
            WeavyLexExprData::Blank => Self::Blank,
            WeavyLexExprData::String(value) => Self::String(value),
            WeavyLexExprData::Pattern { source, flags } => {
            ...
            WeavyLexExprData::Seq(members) => Self::Seq(
                members
                    .into_iter()
                    .map(|member| Self::from_artifact_data(member, compiler))
                    .collect::<Result<Vec<_>, _>>()?,
            ),
            WeavyLexExprData::Choice(members) => Self::Choice(
                members
                    .into_iter()
                    .map(|member| Self::from_artifact_data(member, compiler))
                    .collect::<Result<Vec<_>, _>>()?,
            ),
            WeavyLexExprData::Repeat(content) => {
                Self::Repeat(Box::new(Self::from_artifact_data(*content, compiler)?))
            }
            WeavyLexExprData::Repeat1(content) => {
                Self::Repeat1(Box::new(Self::from_artifact_data(*content, compiler)?))
            }
            WeavyLexExprData::UnsupportedSymbol(expr) => Self::UnsupportedSymbol(expr),
        };
        Ok(match expr {
            Self::Seq(_) | Self::Repeat(_) | Self::Repeat1(_) => {
                Self::compile_composite_regex(expr, compiler)
...
fn regex_source_for_weavy_lex_expr(
    expr: &WeavyLexExpr,
    compiler: &mut WeavyLexerCompiler,
) -> Option<String> {
    match expr {
        ...
        WeavyLexExpr::Pattern { matcher, .. } => matcher.regex_source().map(Cow::into_owned),
        WeavyLexExpr::Seq(members) => {
            let mut source = String::new();
            for member in members {
                source.push_str(&regex_source_for_weavy_lex_expr(member, compiler)?);
            }
            Some(source)
        }
        WeavyLexExpr::Choice(_) => None,
        WeavyLexExpr::Repeat(content) => {
            let source = regex_source_for_weavy_lex_expr(content, compiler)?;
            if source.is_empty() {
                return None;
            }
            Some(format!("(?:{source})*"))
        ...
        WeavyLexExpr::Repeat1(content) => {
            let source = regex_source_for_weavy_lex_expr(content, compiler)?;
            if source.is_empty() || compiler.regex_source_matches_empty(&source) {
                return None;
            }
            Some(format!("(?:{source})+"))
        }
...
fn direct_set_regex_source_for_weavy_lex_expr(
    expr: &WeavyLexExpr,
    compiler: &mut WeavyLexerCompiler,
) -> Option<String> {
    match expr {
        ...
        WeavyLexExpr::Seq(members) => {
            let mut source = String::new();
            for member in members {
                source.push_str(&direct_set_regex_source_for_weavy_lex_expr(
                    member, compiler,
                )?);
            }
            Some(source)
        }
        WeavyLexExpr::Choice(members) => direct_set_choice_regex_source(members, compiler),
        WeavyLexExpr::Repeat(content) => {
            let source = direct_set_regex_source_for_weavy_lex_expr(content, compiler)?;
            if source.is_empty() {
                return None;
            }
            Some(format!("(?:{source})*"))
...
fn direct_set_choice_regex_source(
    members: &[WeavyLexExpr],
    compiler: &mut WeavyLexerCompiler,
) -> Option<String> {
    let mut sources = Vec::with_capacity(members.len());
    for member in members {
        sources.push(direct_set_regex_source_for_weavy_lex_expr(
            member, compiler,
        )?);
    }
    if sources.is_empty() {
...
fn regex_source_matches_empty_compute(source: &str) -> bool {
...
impl From<crate::lex_match::CompiledPattern> for WeavyLexExpr {
    fn from(value: crate::lex_match::CompiledPattern) -> Self {
        let source = value.source.clone();
        let flags = value.flags.clone();
        Self::Pattern {
            matcher: WeavyPatternMatcher::from_compiled_without_cache(value),
            source,
            flags,
        }
    }
...
impl WeavyDirectPatternSet {
    ...
    fn from_terminals(terminals: &mut [WeavyLexTerminal]) -> Option<Self> {
        let mut compiler = WeavyLexerCompiler::default();
        let entries = Self::entries_from_terminals(terminals, &mut compiler);
        let set = Self::from_entries(&entries)?;
        Self::assign_terminal_slots(terminals, &entries);
        Some(set)
    }

    fn entries_from_terminals(
        terminals: &[WeavyLexTerminal],
        compiler: &mut WeavyLexerCompiler,
    ) -> Vec<WeavyDirectPatternSetEntry> {
        let mut entries = Vec::new();
        ...
        for (terminal_index, terminal) in terminals.iter().enumerate() {
            ...
            let Some(regex_sources) = direct_pattern_set_regex_sources(expr, compiler) else {
                continue;
            };
...
impl RuntimeWeavyAutoCloseIndex {
    fn new(parser: &parser_ir::ParserGrammar, lexer_program: &WeavyLexerProgram) -> Self {
        Self::from_lexer_program_with_terminal_count(
            lexer_program,
            parser.symbols().terminals().len(),
        )
    }

    fn from_lexer_program(lexer_program: &WeavyLexerProgram) -> Self {
        let terminal_count = lexer_program
            .modes()
            .flat_map(WeavyLexModeProgram::terminals)
            .map(|terminal| terminal.terminal.get() as usize + 1)
            .max()
            .unwrap_or(0);
        Self::from_lexer_program_with_terminal_count(lexer_program, terminal_count)
    }

    fn from_lexer_program_with_terminal_count(
        lexer_program: &WeavyLexerProgram,
        terminal_count: usize,
    ) -> Self {
        let mut index = Self {
            terminal_specs: vec![None; terminal_count],
            ..Self::default()
        };
...
struct RuntimeWeavyActionEntry {
    ...
    first_action: usize,
    action_count: usize,
    conflict: Option<parser_ir::ConflictId>,
}
...
impl RuntimeWeavyStateActionEntryIndex {
    fn new(table: &parser_ir::ParseTable) -> Self {
        ...
        let states = table
            ...
            .iter()
            .map(|state| {
                RuntimeWeavyStateActionEntries::from_state(state, table, &mut first_action)
            })
            .collect();
        Self { states }
    }
...
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
struct RuntimeWeavyGotoEntry {
    nonterminal: parser_ir::NonterminalId,
    state: parser_ir::ParseStateId,
}

#[derive(Clone, Debug, Default, PartialEq, Eq)]
struct RuntimeWeavyStateGotoIndex {
    states: Vec<RuntimeWeavyStateGotos>,
}

impl RuntimeWeavyStateGotoIndex {
    fn new(table: &parser_ir::ParseTable) -> Self {
        Self {
            states: table
                .states()
                .iter()
                .map(RuntimeWeavyStateGotos::from_state)
                .collect(),
        }
    }
...
impl RuntimeWeavyStateGotos {
    fn from_state(state: &parser_ir::ParseState) -> Self {
        let mut entries = state
            .gotos()
            .iter()
            .map(|goto| RuntimeWeavyGotoEntry {
...
pub(crate) struct BorrowedRuntimeParserFacts<'a> {
    ...
    lex_modes: phon_storage::DenseRange<'a>,
    productions: phon_storage::DenseRange<'a>,
    production_steps: phon_storage::DenseRange<'a>,
...
impl<'a> BorrowedRuntimeParserFacts<'a> {
    ...
    pub(crate) fn conflict_count(&self) -> usize {
        self.header
            .typed_row(0)
            .ok()
            .and_then(|row| row.u32("conflict_count").ok())
...
fn decode_action(row: &phon_storage::DenseRowRef<'_>) -> Option<parser_ir::ParseAction> {
    decode_action_fields(
        row.u8("kind").ok()?,
        row.u32("a").ok()? as usize,
        row.u32("b").ok()? as usize,
        row.u32("c").ok()? as usize,
        row.u32("d").ok()? as usize,
        row.i32("precedence").ok()?,
    )
}
...
impl RuntimeParserFacts for BorrowedRuntimeParserFacts<'_> {
    ...
    fn lex_mode(&self, mode: parser_ir::LexModeId) -> Option<RuntimeLexModeFacts> {
        let row = self.lex_modes.typed_row(mode.get() as usize).ok()?;
        Some(RuntimeLexModeFacts {
            id: mode,
        ...
        for index in 0..range.count() {
            ...
            if row.u32("state").ok()? == state.get() && decode_lookahead(&row)? == lookahead {
                ...
                return Some(RuntimeWeavyActionEntry {
                    ...
                    conflict: optional_id(
                        row.u32("conflict").ok()?,
                        parser_ir::ConflictId::from_index,
                    ),
                });
            }
        }
    ...
    fn state_has_lookahead(
        ...
        self.action_entry(state, lookahead).is_some()
    }
    ...
    fn reserved_terminal(
        ...
        let range = &self.reserved_terminals;
    ...
    fn external_symbol(&self, external: parser_ir::ExternalId) -> Option<RuntimeExternalFacts> {
        let row = self.externals.typed_row(external.get() as usize).ok()?;
        Some(RuntimeExternalFacts {
    ...
    fn production(&self, production: parser_ir::ProductionId) -> Option<RuntimeProductionFacts> {
        self.productions.typed_row(production.get() as usize).ok()?;
        Some(RuntimeProductionFacts)
    }
    ...
    fn production_step(
        ...
        let production_row = self.productions.typed_row(production.get() as usize).ok()?;
    ...
    fn production_metadata(
        ...
        let row = self
            .production_metadata
            .typed_row(metadata.get() as usize)
            .ok()?;
        Some(RuntimeProductionMetadataFacts {
...
impl RuntimeParserFacts for RuntimeParserFactsProvider<'_> {
    ...
    fn action(&self, index: usize) -> Option<parser_ir::ParseAction> {
        match self {
            Self::Owned(facts) => facts.action(index),
            Self::Borrowed(facts) => facts.action(index),
        }
    }
    fn state_count(&self) -> usize {
        match self {
            Self::Owned(facts) => facts.state_count(),
            Self::Borrowed(facts) => facts.state_count(),
        }
    }
    ...
    fn state_has_lookahead(
        ...
        match self {
            Self::Owned(facts) => facts.state_has_lookahead(state, lookahead),
            Self::Borrowed(facts) => facts.state_has_lookahead(state, lookahead),
        }
    }
...
struct RuntimeWeavyOutput<'a> {
    ...
    reuse_scratch: &'a mut RuntimeWeavyReuseScratch,
}

#[derive(Debug, Default)]
struct RuntimeWeavyReuseScratch {
    nodes: HashSet<parser_ir::TreeNodeId>,
    event_indices: Vec<usize>,
    replayed_events: Vec<parser_ir::TreeEvent>,
}
...
pub fn parse_prepared_weavy_collecting_reuse_with_report_and_scanner(
    ...
    parse_weavy_with_lexer_program(
        ...
        None,
        RuntimeWeavyReuseCollection::Enabled,
        runtime_weavy_incremental_block_execution(),
    )
}
...
const fn runtime_weavy_incremental_block_execution() -> RuntimeWeavyBlockExecution {
    #[cfg(all(
        snark_jit_active,
        any(
            all(target_os = "macos", target_arch = "aarch64"),
    ...
    S: RuntimeWeavyDeterministicSink,
{
    ...
    let (stats, accepted) = {
        let mut stepper = RuntimeWeavyStepper::from_branch(
            ...
            RuntimeWeavyMode::Drive,
            S::REUSE_COLLECTION,
        );
        let result = weavy::run_dense_with_stats(&input_ctx.plan.program.dense, &mut stepper);
        let stats = match result {
            Ok(stats) => stats,
            Err(RunError::MissingBlock(block)) => {
                return Err(WeavyParseError::MissingDenseRuntimeBlock {
                    block: block.index(),
                });
            }
            Err(RunError::Step(_)) => return Ok(None),
        };
        let Some(root) = stepper.accepted_root else {
            return Ok(None);
        };
        (
            stats,
            RuntimeWeavyAccepted {
...
struct RuntimeWeavyReuseIndex<'a> {
    ...
    ranged_event_indices: BTreeMap<usize, Vec<usize>>,
    field_event_indices_by_node: HashMap<parser_ir::TreeNodeId, Vec<usize>>,
    edit: parser_ir::ParserInputEdit,
    delta: isize,
...
impl<'a> RuntimeWeavyReuseIndex<'a> {
    fn from_report(report: &'a WeavyParseReport, edit: parser_ir::ParserInputEdit) -> Self {
        ...
        for (index, event) in report.tree_events.iter().enumerate() {
            ...
            if let parser_ir::TreeEvent::Field { node, .. } = event {
                field_event_indices_by_node
                    .entry(*node)
                    .or_default()
                    .push(index);
            }
...
fn parse_weavy_with_lexer_program_and_scratch(
    ...
    let mut tree_store = reuse_index.map_or_else(
        || {
            RuntimeWeavyTreeStore::with_capacity(
                runtime_weavy_initial_tree_node_capacity(input_ctx.input),
                runtime_weavy_initial_child_list_capacity(input_ctx.input),
            )
        },
        |reuse_index| reuse_index.tree_store.clone(),
    );
    ...
    let mut reuse_scratch = RuntimeWeavyReuseScratch::default();
    ...
    let step_limit = match recovery {
        RuntimeWeavyRecoveryMode::Strict => {
            runtime_weavy_step_limit(input_ctx.facts.state_count(), input_ctx.input)
        }
        RuntimeWeavyRecoveryMode::SkipInvalidInput => {
            runtime_weavy_recovery_step_limit(input_ctx.facts.state_count(), input_ctx.input)
        }
    };
    ...
    }) = branches.pop()
    {
        ...
        let mut output = RuntimeWeavyOutput {
            tree_store: &mut tree_store,
            trace_events: &mut trace_events,
            reuse_scratch: &mut reuse_scratch,
            tree_journal: &mut tree_journal,
...
fn runtime_weavy_step_limit(state_count: usize, input: &str) -> usize {
    let input_budget = input.len().saturating_mul(4096);
    let table_budget = state_count.saturating_mul(64);
    10_000usize.max(input_budget.saturating_add(table_budget))
}

fn runtime_weavy_recovery_step_limit(state_count: usize, input: &str) -> usize {
...
fn runtime_weavy_actions(
    facts: RuntimeParserFactsProvider<'_>,
    first_action: usize,
    action_count: usize,
    state: parser_ir::ParseStateId,
    lookahead: parser_ir::LookaheadSymbol,
    byte_position: usize,
) -> Result<Vec<parser_ir::ParseAction>, RuntimeWeavyStepError> {
    let actions = (0..action_count)
        .map(|index| facts.action(first_action.checked_add(index)?))
        .collect::<Option<Vec<_>>>();
    actions.ok_or(RuntimeWeavyStepError::NoAction {
        state,
        lookahead,
        byte_position,
    })
...
fn step_runtime_weavy_branch(
    ...
    if actions.len() > 1 {
        ...
        let versions = (0..actions.len())
            ...
            .collect::<Vec<_>>();
        let Some(conflict) = dispatch.conflict else {
            step.outcomes.push(RuntimeWeavyStepOutcome::Failed {
                version: source_version,
                failure: runtime_weavy_failure(
                    RuntimeWeavyStepError::UnsupportedCanonicalOp,
                    branch_state_stack,
                ),
            });
            return;
...
fn try_reuse_runtime_weavy_node(
    ...
    let goto_state = input_ctx
        .plan
        .program
        .goto_state(entry_state, reusable.symbol)?;
    let node = reusable.source_node;
    let (bytes, points) =
    ...
    replay_reused_runtime_weavy_tree_events(
        reuse_index,
        reusable,
        branch.version,
        output.input_points,
        output.reuse_scratch,
    );
    if output.trace_events.is_enabled() {
        output
            .tree_journal
            .push(&mut branch.tree_journal, tree_event.clone());
        output.tree_journal.extend(
            &mut branch.tree_journal,
            output.reuse_scratch.replayed_events.iter().cloned(),
        );
        output
            .trace_events
...
fn recover_runtime_weavy_to_action_state(
    ...
    for len in (1..original_len).rev() {
        let state = branch.stack[len - 1].state;
        if facts.state_has_lookahead(state, lookahead) {
            branch.stack.truncate(len);
            branch.error_cost = branch
                .error_cost
                .saturating_add(u32::try_from(original_len - len).unwrap_or(u32::MAX));
...
impl<'a> RuntimeWeavyStepper<'a> {
    ...
    fn probe_state_direct(
        ...
        Ok(RuntimeWeavyDispatch {
            ...
            conflict: action_entry.conflict,
        })
    }
    ...
    fn shift_direct(
        ...
        if self.collects_runtime_tree_events() {
            let (bytes, points) = runtime_weavy_input_ranges(self.input_points, start, token.end);
            self.emit_runtime_tree_event(parser_ir::TreeEvent::Token {
                version: self.version,
    ...
    ) -> Result<Control<'program, BlockRef, DenseSnarkWeavyOp, BlockRef>, RuntimeWeavyStepError>
    {
        ...
        match intrinsic {
            ...
            SnarkIntrinsic::DispatchActions { state, .. } => {
                ...
                let Some(action) = self.facts.action(action_entry.first_action) else {
                    return Err(RuntimeWeavyStepError::NoAction {
                        state,
                        lookahead: token.lookahead,
                        byte_position: self.byte_position,
    ...
    fn lex(
            ...
            |direct_literal_ends,
             ...
             -> Result<(), RuntimeWeavyStepError> {
                    ...
                    .chain(mode_program.non_direct_terminal_indices.iter().copied())
                {
                    ...
                    let mut candidate = RuntimeWeavyTokenCandidate {
                    ...
                    if candidate_is_word && reserved_context.is_some() {
                        if candidate.keyword == parser_ir::KeywordStatus::Rewritten {
                            candidate.extra = terminal_row
                                .lookahead
                                .is_some_and(|lookahead| lookahead.shifts_only_extra);
                            push_runtime_weavy_candidate(&mut best, candidate);
                            continue;
                        }
                        candidate.keyword = parser_ir::KeywordStatus::Word;
                    }
                ...
                if let (Some(word), Some(context), Some(reserved_candidate)) =
                    ...
                    && let Some(word_index) = mode_program
                        .terminals()
                        .iter()
                        .position(|terminal| terminal.terminal == word)
                    && let Some(word_match) = self.match_compiled_terminal_with_set(
                        &mode_program.terminals()[word_index],
                        byte_position,
                        direct_literal_ends,
                        direct_pattern_ends,
                    )?
                    ...
                    && reserved_candidate.end == word_match.end
                {
                    ...
                    push_runtime_weavy_candidate(
                        ...
                        RuntimeWeavyTokenCandidate {
                            ...
                            literal: word_row.literal,
                            lexical_precedence: word_row.lexical_precedence,
                            implicit_precedence: word_row.implicit_precedence,
        ...
        if self.external_scanner.is_none() && !mode_program.external_lookaheads.is_empty() {
    ...
    fn collects_runtime_tree_events(&self) -> bool {
        self.tree_event_collection == RuntimeWeavyTreeEventCollection::Enabled
    }
    ...
    fn runtime_extra_fragment(
        ...
        if self.collects_runtime_tree_events() {
            ...
            self.emit_runtime_tree_event(parser_ir::TreeEvent::CloseNode {
                version,
                node,
...
struct RuntimeWeavyToken {
    ...
    inspected_end: usize,
    keyword: parser_ir::KeywordStatus,
...
fn match_weavy_direct_pattern_fallback(
    expr: &WeavyLexExpr,
    input: &str,
    byte_position: usize,
) -> Option<parser_ir::LexMatch> {
    match expr {
        WeavyLexExpr::Pattern { matcher, .. } => matcher.match_input(input, byte_position),
        _ => {
            let scratch = RuntimeWeavyLexerScratch::default();
            match_weavy_lex_expr_runtime_inner(expr, input, byte_position, &scratch, false)
                .ok()
                .flatten()
        }
    }
...
fn direct_pattern_set_regex_sources(
    expr: &WeavyLexExpr,
    compiler: &mut WeavyLexerCompiler,
) -> Option<Vec<String>> {
    match expr {
        WeavyLexExpr::Pattern { matcher, .. } => matcher
            .regex_source()
            .map(|source| vec![source.into_owned()]),
        WeavyLexExpr::CompositeRegex { matcher, .. } => {
            let source = matcher.source().to_owned();
            (!compiler.regex_source_matches_empty(&source)).then_some(vec![source])
        }
        WeavyLexExpr::CompositeChoice { original, .. } => match original.as_ref() {
            WeavyLexExpr::Choice(members) => {
                let mut sources = Vec::with_capacity(members.len());
                for member in members {
                    let source = direct_set_regex_source_for_weavy_lex_expr(member, compiler)?;
                    if source.is_empty() {
                        return None;
                    }
                    sources.push(source);
        ...
        _ => {
            let source = direct_set_regex_source_for_weavy_lex_expr(expr, compiler)?;
            (!compiler.regex_source_matches_empty(&source)).then_some(vec![source])
        }
    }
}
...
fn runtime_weavy_subtree_tree_event_indices(
    reuse_index: &RuntimeWeavyReuseIndex<'_>,
    start_byte: usize,
    end_byte: usize,
    root_node: parser_ir::TreeNodeId,
    nodes: &mut HashSet<parser_ir::TreeNodeId>,
    event_indices: &mut Vec<usize>,
) {
    nodes.clear();
    nodes.insert(root_node);
    event_indices.clear();

    for indices in reuse_index
        .ranged_event_indices
    ...
    for node in nodes.iter() {
        let Some(indices) = reuse_index.field_event_indices_by_node.get(node) else {
            continue;
        };
        for index in indices {
            let parser_ir::TreeEvent::Field { child, .. } = &reuse_index.tree_events[*index] else {
                unreachable!("field event index contains a non-field event");
            };
            if child.is_none_or(|child| nodes.contains(&child)) {
                event_indices.push(*index);
            }
        }
    }
...
fn replay_reused_runtime_weavy_tree_events(
    ...
    version: parser_ir::StackVersionId,
    input_points: &RuntimeWeavyInputPoints,
    scratch: &mut RuntimeWeavyReuseScratch,
) {
    runtime_weavy_subtree_tree_event_indices(
        reuse_index,
        reusable.source_start_byte,
        reusable.source_end_byte,
        reusable.source_node,
        &mut scratch.nodes,
        &mut scratch.event_indices,
    );
    scratch.replayed_events.clear();
    scratch.replayed_events.reserve(scratch.event_indices.len());
    for index in scratch.event_indices.iter().copied() {
        let event = runtime_weavy_shift_tree_event_bytes(
            reuse_index.tree_events[index].clone(),
            reuse_index.edit,
            reuse_index.delta,
        );
        scratch
            .replayed_events
            .push(replay_reused_runtime_weavy_tree_event(
                &event,
                version,
                input_points,
            ));
...
fn match_weavy_lex_expr_runtime_inner(
    ...
    match expr {
        ...
        WeavyLexExpr::Pattern { matcher, .. } => {
            ...
            Ok(matcher.match_input_with_scratch(input, byte_position, lexer_scratch))
        }
...
mod tests {
    ...
    fn prepared_reserved_grammar(
        ...
        let raw = crate::grammar::RawGrammarJson::from_tree_sitter_json_str(grammar_json)
            .expect("import reserved-word grammar");
        let validated = ValidatedGrammar::from_raw(&raw).expect("validate reserved-word grammar");
    ...
    fn reserved_context_grammar_json(
        identifier_pattern: &str,
        keyword_pattern: &str,
        keyword_flags: Option<&str>,
        reserved_sets: &str,
        second_context: Option<&str>,
        allow_quoted: bool,
    ) -> String {
        let keyword_flags = keyword_flags
            .map(|flags| format!(r#", "flags": "{flags}""#))
            .unwrap_or_default();
        let reserved_content = if allow_quoted {
            r#"{
              "type": "CHOICE",
              "members": [
                { "type": "SYMBOL", "name": "identifier" },
                { "type": "SYMBOL", "name": "quoted_identifier" }
              ]
            }"#
        } else {
            r#"{ "type": "SYMBOL", "name": "identifier" }"#
        };
        let second_branch = second_context
            .map(|context| {
                format!(
                    r#", {{
                      "type": "SEQ",
        ...
        format!(
            r#"{{
              ...
              "rules": {{
                ...
                "keyword": {{
                  "type": "PATTERN",
                  "value": "{keyword_pattern}"{keyword_flags}
                }},
                "identifier": {{
                  "type": "TOKEN",
                  "content": {{ "type": "PATTERN", "value": "{identifier_pattern}" }}
                }},
                "quoted_identifier": {{
                  "type": "TOKEN",
                  "content": {{
    ...
    #[test]
    fn reserved_word_is_identifier_without_context_and_keyword_with_context() {
        let grammar_json = reserved_context_grammar_json(
            "[A-Za-z_][A-Za-z0-9_]*",
            "row",
            None,
            r#"{
                "strict": [{ "type": "PATTERN", "value": "row" }]
            }"#,
            None,
            false,
        );
    ...
    #[test]
    fn reserved_keyword_does_not_steal_a_longer_identifier_prefix() {
        let grammar_json = reserved_context_grammar_json(
            "[A-Za-z_][A-Za-z0-9_]*",
            "row",
            None,
        ...
        parse_prepared_weavy_with_report(&plan, &parser, &table, "reservedrow_number")
            .expect("longest identifier match wins over the keyword prefix");
    }

    #[test]
    fn reserved_keyword_matching_respects_case_insensitive_pattern_flags() {
        let grammar_json = reserved_context_grammar_json(
            "[A-Za-z_][A-Za-z0-9_]*",
            "row",
            Some("i"),
            r#"{
                "strict": [{ "type": "PATTERN", "value": "row", "flags": "i" }]
            }"#,
            None,
            false,
        );
        ...
        parse_prepared_weavy_with_report(&plan, &parser, &table, "reservedROW")
            .expect("case-insensitive keyword rewrites in the active context");
    }

    #[test]
    fn multiple_reserved_sets_select_only_the_active_context_keywords() {
        let grammar_json = reserved_context_grammar_json(
            "[A-Za-z_][A-Za-z0-9_]*",
            "row|value",
            None,
            r#"{
                "strict": [{ "type": "PATTERN", "value": "row" }],
                "other": [{ "type": "PATTERN", "value": "value" }]
            }"#,
            Some("other"),
            false,
        );
        ...
        parse_prepared_weavy_with_report(&plan, &parser, &table, "reservedrow")
            .expect("strict context reserves row");
        parse_prepared_weavy_with_report(&plan, &parser, &table, "reservedvalue")
            .expect("strict context leaves the other set's spelling as an identifier");
        parse_prepared_weavy_with_report(&plan, &parser, &table, "othervalue")
            .expect("other context reserves value");
        parse_prepared_weavy_with_report(&plan, &parser, &table, "otherrow")
            .expect("other context leaves the strict set's spelling as an identifier");
    }

    #[test]
    fn quoted_identifier_is_unaffected_by_reserved_word_contexts() {
        let grammar_json = reserved_context_grammar_json(
            "[A-Za-z_][A-Za-z0-9_]*",
            "row",
            None,
        ...
        parse_prepared_weavy_with_report(&plan, &parser, &table, "reserved\"row\"")
            .expect("quoted identifiers bypass reserved-word rewriting");
    }
    ...
    fn known_patterns_lower_to_named_weavy_leaves() {
        let matcher = WeavyPatternMatcher::from_compiled_without_cache(
            crate::lex_match::compile_pattern("and\\b", None),
        );
    ...
    fn direct_pattern_set_reports_all_overlapping_root_matches() {
        let mut terminals = vec![
            WeavyLexTerminal {
                terminal: parser_ir::TerminalId::from_index(0),
                matcher: WeavyTerminalMatcher::Expr(
                    crate::lex_match::compile_pattern("[a-z]+", None).into(),
                ),
                lookahead: None,
                immediate: false,
    ...
    #[test]
    fn direct_pattern_set_covers_composite_regex_terminals() {
        let mut compiler = WeavyLexerCompiler::default();
        ...
        assert!(matches!(composite, WeavyLexExpr::CompositeRegex { .. }));
        let mut terminals = vec![WeavyLexTerminal {
            terminal: parser_ir::TerminalId::from_index(0),
        ...
        assert_eq!(mode.direct_pattern_set.as_ref().unwrap().len(), 1);
        let mut ends = Vec::new();
        let mut matches = None;
        let mut dfa_cache = mode
            .direct_pattern_set
    ...
    fn parse_plan_analysis_combines_parser_and_lexer_shapes() {
        ...
        let mut expected_backend_state = lex
            .semantics()
            .stencil
            .state
            .iter()
    ...
    #[test]
    fn regex_emptiness_cache_matches_direct_compute() {
        // The memoized ε-membership test must agree with the uncached regex_automata
        // build so the lexer-compiler cache never changes which `+`/repeat patterns are
        // accepted. Covers empty-matching and non-empty-matching sources.
        let sources = [
            "a*", "(?:ab)*", "[0-9]*", "(?:a|b)*", "(?:a*)+", "a+", "abc", "\\w+", "[0-9]+",
            "(?:a|b)+",
        ];
        let mut compiler = WeavyLexerCompiler::default();
        for source in sources {
            let cached = compiler.regex_source_matches_empty(source);
            let direct = regex_source_matches_empty_compute(source);
            assert_eq!(
                cached, direct,
                "cache disagrees with direct compute for {source:?}"
            );
            // A repeated call must hit the cache and return the identical value.
            assert_eq!(
                compiler.regex_source_matches_empty(source),
                cached,
                "cache inconsistent across calls for {source:?}"
            );
        }
        // Anchored expectations: a star matches empty; a plus / literal does not.
        assert!(compiler.regex_source_matches_empty("a*"));
        assert!(!compiler.regex_source_matches_empty("a+"));
        assert!(!compiler.regex_source_matches_empty("abc"));
    }

    #[test]
    fn identical_regex_sources_share_a_deduped_leaf() {
        // Repeated composite/pattern leaves with the same source must reuse one compiled
        // engine (same cache id) and behave identically at match time.
        let mut compiler = WeavyLexerCompiler::default();
        let first = compiler
            .regex_leaf("[a-z]+".to_string(), None)
            .expect("first leaf compiles");
        let second = compiler
            .regex_leaf("[a-z]+".to_string(), None)
            .expect("deduped leaf compiles");
        assert_eq!(
            first.cache_id, second.cache_id,
            "identical source should share one leaf engine"
        );
        assert_eq!(first.source(), second.source());
        let input = "hello world";
        assert_eq!(first.match_input(input, 0), second.match_input(input, 0));
        // Distinct sources stay distinct.
        let other = compiler
            .regex_leaf("[0-9]+".to_string(), None)
            .expect("other leaf compiles");
